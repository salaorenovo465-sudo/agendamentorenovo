...
const TENANT_REGISTRY_KEY = 'tenants_registry';
const LEGACY_SETTINGS_KEY = 'general';
const TENANT_SETTINGS_PREFIX = 'tenant:';
const TENANT_SLUG_REGEX = /^[a-z0-9][a-z0-9-]{1,48}$/;
const DEFAULT_TENANT_SLUG = (process.env.DEFAULT_TENANT_SLUG || 'renovo').toLowerCase();
const DEFAULT_TENANT_NAME = process.env.DEFAULT_TENANT_NAME || 'Estudio Renovo';

const normalizeTenantSlug = (raw: string | null | undefined): string | null => {
  if (!raw) return null;
...
const normalizeTenantName = (raw: string | null | undefined): string => {
...
const mapTenantRegistry = (value: unknown): TenantRecord[] => {
...
const toTenantRegistryValue = (tenants: TenantRecord[]): TenantRegistryValue => ({
...
const settingsKeyForTenant = (tenantSlug: string | null | undefined): string => {
