...
const parseLabelsJson = (value: unknown): string[] => {
  if (typeof value !== 'string' || !value.trim()) {
    return [];
  }
...
const mapSqliteMeta = (row: Record<string, unknown>): ConversationMeta => ({
...
const mapSupabaseMeta = (row: Record<string, unknown>): ConversationMeta => ({
  threadId: Number(row.thread_id),
  assigneeId: row.assignee_id ? String(row.assignee_id) : null,
  status: normalizeStatus(row.status),
...
const mapSqliteNote = (row: Record<string, unknown>): InternalNote => ({
  id: Number(row.id),
  threadId: Number(row.thread_id),
...
const mapSupabaseNote = (row: Record<string, unknown>): InternalNote => ({
  id: Number(row.id),
  threadId: Number(row.thread_id),
  content: String(row.content || ''),
  author: row.author ? String(row.author) : null,
...
const normalizeSyncStatus = (value: unknown): WhatsappSyncStatus => {
  const raw = typeof value === 'string' ? value.trim().toLowerCase() : '';
...
const mapSqliteSyncState = (row: Record<string, unknown>): WhatsappSyncState => ({
  id: Number(row.id),
...
const mapSupabaseSyncState = (row: Record<string, unknown>): WhatsappSyncState => ({
  id: Number(row.id),
  scope: String(row.scope || ''),
  source: String(row.source || ''),
  cursor: row.cursor ? String(row.cursor) : null,
...
const mapSqliteContactMap = (row: Record<string, unknown>): WhatsappContactMap => ({
  id: Number(row.id),
...
const mapSupabaseContactMap = (row: Record<string, unknown>): WhatsappContactMap => ({
  id: Number(row.id),
  threadId: row.thread_id ? Number(row.thread_id) : null,
  phone: String(row.phone || ''),
  waJid: row.wa_jid ? String(row.wa_jid) : null,
...
const mapSqliteConversationMap = (row: Record<string, unknown>): WhatsappConversationMap => ({
  id: Number(row.id),
  threadId: Number(row.thread_id),
  waJid: row.wa_jid ? String(row.wa_jid) : null,
...
const mapSupabaseConversationMap = (row: Record<string, unknown>): WhatsappConversationMap => ({
  id: Number(row.id),
  threadId: Number(row.thread_id),
  waJid: row.wa_jid ? String(row.wa_jid) : null,
  evolutionChatId: row.evolution_chat_id ? String(row.evolution_chat_id) : null,
