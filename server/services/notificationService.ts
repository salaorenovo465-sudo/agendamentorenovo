...
const buildRejectedBookingMessage = (booking: BookingRecord): string => [
  ...
  '',
  `Ola, *${booking.name}*! \u{1F496}`,
  'Somos do *Estudio Renovo* e, neste momento, nao conseguimos confirmar o seu horario. \u23F3',
  '',
  '\u{1F451} *SERVICO:*',
  resolveBookingServiceLabel(booking),
  '',
  `\u{1F4C5} *DATA:* ${formatBookingDate(booking.date)}`,
  `\u23F0 *HORARIO:* ${booking.time}`,
  booking.rejectionReason ? `\u{1F4DD} *MOTIVO:* ${booking.rejectionReason}` : '',
  '',
  'Se quiser, responda esta mensagem para encontrarmos um novo horario com carinho. \u{1F4AC}\u2728',
  'Agradecemos pela compreensao! \u{1F495}',
].filter(Boolean).join('\n');

const buildRescheduledBookingMessage = (booking: BookingRecord): string => [
  '\u{1F504} *AGENDAMENTO REMARCADO* \u{1F504}',
  '',
  `Ola, *${booking.name}*! \u{1F496}`,
  'Seu horario no *Estudio Renovo* foi atualizado com sucesso. \u2728',
  '',
  '\u{1F451} *SERVICO:*',
  resolveBookingServiceLabel(booking),
  '',
  `\u{1F4C5} *NOVA DATA:* ${formatBookingDate(booking.date)}`,
  `\u23F0 *NOVO HORARIO:* ${booking.time}`,
  '',
  'Estamos te esperando para cuidar de voce com todo carinho. \u{1F338}\u{1F495}',
  'Se precisar de outro ajuste, responda esta mensagem. \u{1F4AC}',
].join('\n');
